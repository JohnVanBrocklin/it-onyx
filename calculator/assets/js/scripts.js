function calculate() {
    ...
    if ($( "#myform" ).valid()) {
        ...
        /* if the operator was "Add" then set result to the minimum */
        if (operator == "Add") {
            result = operand1fp + operand2fp
        }
 
        /* if the operator was "Sub" then set result to the maximum */
        if (operator == "Sub") {
            result = operand1fp - operand2fp
        }

        /* if operator was "Mul" the calcualute the average of 3 operands */
        if (operator == "Mul") {
            result = operand1fp * operand2fp;
        }
        
        /* if operator was "Div" the calcualute the average of 3 operands */
        if (operator == "Div") {
            result = operand1fp / operand2fp;
        }
